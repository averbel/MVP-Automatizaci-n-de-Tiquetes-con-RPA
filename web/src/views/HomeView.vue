    ...
    <div v-if="estado === 'FORM'" class="relative z-10 container mx-auto px-4 py-12 lg:py-24 min-h-screen flex flex-col lg:flex-row items-center gap-12">
      <div class="lg:w-1/2 space-y-8 animate-fade-in-up text-white">
        ...
        <p class="text-xl text-slate-300 max-w-xl font-light leading-relaxed">
          Solicita tu vuelo y nuestra IA avanzada se encargara de encontrar la mejor opcion dentro de nuestra politica corporativa.
        </p>
      </div>
          ...
          <div class="p-8 sm:p-12 space-y-8">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-8">
              ...
              <button type="submit" :disabled="loading" class="w-full flex justify-center items-center py-4 px-6 rounded-xl text-lg font-bold text-white transition-all shadow-xl hover:shadow-blue-500/30 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 shadow-[0_0_15px_rgba(6,182,212,0.4)] hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div v-else>Solicitar Autorizacion</div>
              </button>
            </form>
          </div>
        </div>
              ...
              <div class="flex justify-between items-center text-sm text-slate-400">
                ...
                <div class="flex-grow text-center px-4">
                  ...
                  <div class="h-px bg-slate-300 w-full relative my-2">
                    <span class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-slate-400">&#9992;</span>
                  </div>
                </div>
                <div class="text-center">
...
const startPolling = () => {
  pollInterval = setInterval(async () => {
    try {
      ...
      if (data.estado === 'OPCIONES_LISTAS' && data.opciones?.length > 0) {
        opcionesVuelo.value = data.opciones;
        estado.value = 'SELECCIONANDO_VUELO';
        stopPolling();
      } else if (data.estado === 'SIN_OPCIONES' || data.estado === 'FALLIDA') {
        estado.value = 'SIN_OPCIONES';
        stopPolling();
      }
    } catch { /* retry */ }
...
const comprarVuelo = async (vuelo: any) => {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Error al lanzar RPA');
    ...
    pollInterval = setInterval(async () => {
      try {
        ...
        if (d.estado_rpa === 'EXITO') {
          capturaResult.value = d.capturas?.[d.capturas.length - 1] || null;
          estado.value = 'RPA_COMPLETADO';
          stopPolling();
        } else if (d.estado_rpa === 'FALLO') {
          alert('El bot fallo al realizar la compra.');
          stopPolling();
          resetFlow();
        }
      } catch { /* retry */ }
    ...
    alert('Error de conexion al comprar.');
    resetFlow();
  }
};
